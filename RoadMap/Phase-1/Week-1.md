# ✅ Week-1 Checklist — Foundations + Security Risks

## 🎯 Weekly Goal
Understand cybersecurity fundamentals, roles, frameworks, and security tools.

---

## [[Day-1]] 
**Introduction to Exciting World of Cybersecurity**
- [x] What is cybersecurity? ✅ 2025-11-04
- [x] Why organizations need security ✅ 2025-11-04
- [x] Core job roles overview ✅ 2025-11-04

---

## [[Day-2 ]] 
**The Evolution of Cybersecurity**
- [ ] Viruses and Malware
- [ ] Social Engineering and Digital Age of Threat Actors
- [ ] CIA Triad (Confidentiality, Integrity, Availability)
- [ ] Authentication vs Authorization
- [ ] Least privilege & Zero Trust basics
- [ ] Real-world examples for each

📝 Notes:
- AuthN vs AuthZ table

---

## [[Day-03 - Security in the Real World]]
- [ ] Review major cyber breaches
- [ ] Impact on business operations
- [ ] Compliance importance
- [ ] Add 1 case study

📝 Notes:
- What failure caused the breach?

---

## [[Day-04 - Threats Vulnerabilities Risks]]
- [ ] Define: Asset / Threat / Vulnerability / Risk
- [ ] Threat categories (internal/external)
- [ ] Everyday examples of risk

📝 Notes:
- Risk = Threat × Vulnerability × Impact

---

## [[Day-05 - Security Frameworks Controls]]
- [ ] NIST CSF (5 functions)
- [ ] Admin / Technical / Physical controls
- [ ] Examples of controls per category

📝 Notes:
- Detect vs Protect vs Respond mapping

---

## [[Day-06 - Security Tools Overview]]
- [ ] Antivirus (AV)
- [ ] Firewalls
- [ ] EDR/XDR
- [ ] SIEM basics & use cases

📝 Notes:
- Tool → SOC purpose mapping

---

## [[Day-07 - Week Review]]
- [ ] Revisit quizzes
- [ ] Glossary creation
- [ ] Organize notes
- [ ] Identify favorite topics

📝 Reflection:
- What did I learn?
- What do I want to go deeper into?

---

✅ **End of Week-1 Outcomes**
- Industry vocabulary and key concepts
- Understanding frameworks & risk
- Foundation for network + Linux learning
